...
class Depends_Command extends Command {
	...
	public function __invoke( $args, $assoc_args ) {
		...
		// Resolve short class/interface names from cache.
		...
		$config     = null !== $this->cache_data ? $this->cache_data['bindings'] : $this->load_config( $path );
		$dependents = $this->find_dependents( $class_name, $config );
		$rows       = array();
		...
		if ( $this->is_data_format() ) {
			$this->format_items( $rows, array( 'type', 'class', 'param', 'config mapping' ) );
	...
	/**
	 * Find all concrete classes in the cache that depend on the target class.
	 *
	 ...
	 */
	private function find_dependents( string $target_fqcn, array $config ): array {
