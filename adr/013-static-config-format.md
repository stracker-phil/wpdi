...
Conditional bindings (e.g. environment-dependent implementation selection) belong in a `ServiceProvider` class resolved from `bootstrap()`, not in the config file.
...
- No escape hatch for passing scalar values to constructors via config; use a `ServiceProvider` instead
